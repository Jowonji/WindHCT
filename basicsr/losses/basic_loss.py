...
class DISTSPerceptualLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # DISTS may return a per-sample score, so take the mean
...
class PerceptualLoss(nn.Module):
    ...
    Args:
        layer_weights (dict): The weight for each layer of VGG feature.
            Example: {'conv5_4': 1.0} means the conv5_4 feature layer is
            used to calculate the loss with weight 1.0.
        ...
        perceptual_weight (float): Weight for the perceptual loss. Default: 1.0.
        style_weight (float): Weight for the style loss. Default: 0.
    ...
    def __init__(self,
        ...
        if self.criterion_type == 'l1':
            ...
            self.criterion = None  # computed directly via torch.norm below
        else:
...
class PatchSimilarityLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        return (loss / count if count > 0 else loss) * self.loss_weight
...
@LOSS_REGISTRY.register()
class CharbonnierLoss(nn.Module):
    def __init__(self, eps=1e-3, loss_weight=1.0):
        super(CharbonnierLoss, self).__init__()
        self.eps = eps
        self.loss_weight = loss_weight
